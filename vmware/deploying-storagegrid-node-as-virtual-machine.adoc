...
NOTE: La contraseña de instalación temporal solo se utiliza durante la instalación del nodo. Después de añadir un nodo a la grid, puedes acceder a él usando la link:../admin/change-node-console-password.html["contraseña de la consola del nodo"], que aparece en el archivo  `Passwords.txt` del Recovery Package.
