...
** En raras ocasiones, como cuando se lo indica el soporte técnico o cuando sabe que el nodo de reemplazo tiene menos volúmenes disponibles para el almacenamiento de objetos que el nodo original, debe link:restoring-object-data-to-storage-volume.html["restaurar datos de objetos manualmente"] con el `repair-data` guión. Si se aplica uno de estos casos, responda `n`.
+
[NOTE]
====
